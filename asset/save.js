...
const sharp = require("sharp"); // stuff
...
module.exports = async function (req, res, url) {
  ...
  switch (url.pathname) {
    case "/api/asset/upload":
      {
        ...
        const ext = origName.substring(dotIn + 1); // read the file
          ...
          case "bg":
            {
              await new Promise((resolve, rej) => {
                ...
                stream.on("end", async () => {
                  ...
                  const buffer = await sharp(buf).resize(550, 354, {
                    fit: "fill"
                  }).toBuffer();
          ...
          case "prop":
            {
              ...
              if (meta.subtype == "video") {
                ...
                await new Promise((resolve, rej) => {
                  ...
                  stream.resume();
                  stream.pipe(writeStream); // convert the video to an flv

                  ffmpeg(oldPath).ffprobe((e, data) => {
                    if (e) rej(e);
                    meta.width = data.streams[0].width;
                    meta.height = data.streams[0].height; // convert the video to an flv

                    ffmpeg(oldPath).output(newPath).on("end", () => {
                      const buffer = fs.readFileSync(newPath);
                      aId = asset.save(buffer, meta); // save the first frame

                      ffmpeg(oldPath).seek("0:00").output(`${process.env.ASSET_FOLDER}/${aId.slice(0, -3) + "png"}`).outputOptions("-frames", "1").on("end", () => resolve(aId)).run();
                    }).on("error", e => rej("Error converting video:", e)).run();
                  });
                });
    ...
    case "/goapi/saveSound/":
      {
        // sound recording/uploading through the lvm
        ...
        if (isRecord) {
          ...
          oldStream = fs.createReadStream(path); // get the extension
        ...
        stream.on("end", () => {
          ...
          mp3Duration(buf, (e, duration) => {
            ...
            res.end(`0<response><asset><id>${aId}</id><enc_asset_id>${aId}</enc_asset_id><type>sound</type><subtype>${meta.subtype}</subtype><title>${meta.title}</title><published>0</published><tags></tags><duration>${meta.duration}</duration><downloadtype>progressive</downloadtype><file>${aId}</file></asset></response>`);
          });
          return true;
        });
      }
